...
export default function ProductOverviewClient() {
  ...
  // ── Check Razorpay SDK availability (loaded globally via layout) ──────────
  // Script uses lazyOnload so it arrives after mount — poll until available.
  useEffect(() => {
    if (typeof window === "undefined") return;
    if ((window as any).Razorpay) { setSdkReady(true); return; }
    const id = setInterval(() => {
      if ((window as any).Razorpay) { setSdkReady(true); clearInterval(id); }
    }, 300);
                  ...
                  <>
                    ...
                    <div className="px-8 py-6 overflow-y-auto" style={{ maxHeight: "calc(90vh - 120px)" }}>
                      ...
                      <div className="mb-5">
                        ...
                        <div className="mb-2">
                          <label htmlFor="field-name" className="sr-only">Full Name</label>
                          <input id="field-name" placeholder="Full Name *" value={name} onChange={(e) => { setName(e.target.value); clearError("name"); }} className={inputCls(errors.name)} style={SANS} />
                          <ErrMsg msg={errors.name} />
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <label htmlFor="field-phone" className="sr-only">Mobile</label>
                            <input id="field-phone" placeholder="Mobile *" value={phone} onChange={(e) => { setPhone(e.target.value); clearError("phone"); }} className={inputCls(errors.phone)} style={SANS} />
                            <ErrMsg msg={errors.phone} />
                          </div>
                          <div>
                            <label htmlFor="field-email" className="sr-only">Email</label>
                            <input id="field-email" placeholder="Email *" value={email} onChange={(e) => { setEmail(e.target.value); clearError("email"); }} className={inputCls(errors.email)} style={SANS} />
                      ...
                      <div className="mb-5">
                        ...
                        <div className="mb-2">
                          <label htmlFor="field-address1" className="sr-only">Address Line 1</label>
                          <input id="field-address1" placeholder="Address Line 1 *" value={address1} onChange={(e) => { setAddress1(e.target.value); clearError("address1"); }} className={inputCls(errors.address1)} style={SANS} />
                          <ErrMsg msg={errors.address1} />
                        </div>
                        <div className="mb-2">
                          <label htmlFor="field-address2" className="sr-only">Address Line 2</label>
                          <input id="field-address2" placeholder="Address Line 2 (Optional)" value={address2} onChange={(e) => setAddress2(e.target.value)} className="input rounded-sm w-full" style={SANS} />
                        </div>
                      ...
                      <div className="mb-4">
                        ...
                        <label htmlFor="field-notes" className="sr-only">Delivery Notes</label>
                        <textarea id="field-notes" className="input h-16 rounded-sm resize-none w-full" placeholder="Instructions..." value={notes} onChange={(e) => setNotes(e.target.value)} style={SANS} />
                      </div>
